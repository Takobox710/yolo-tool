...
- 普通 Git 提交的 body 必须逐字等于归并后 CHANGELOG 提交条目下的正文，包含相同的项目符号、顺序和具体内容。整理时先按最终用户结果、维护价值和功能链路归并：同一结果、同一链路或同一实现目的应合并；重复、被覆盖、仅属实现细节或没有独立维护价值的内容应删除；正常提交最终保留 3 至 6 条项目符号，少于 3 条时只能按真实的用户结果、维护结果或发布结果拆分，不得凭空凑数；超过 6 条时必须继续按功能链路归并。不要为了追求简短合并独立改动，也不要机械复制原始记录；不要求原始待提交记录与最终正文一一对应。每条文字建议控制在 25 至 50 个字，少于 25 个字时补充关键对象、结果或影响，最多不超过 70 个字；超过 70 个字必须继续压缩或合并。最终摘要应整体简洁，不只是压缩单条文字。正文允许改写但不得改变事实；提交前后必须核对 CHANGELOG 与 Git body 完全一致，不一致时必须停止提交并修正。
...
- 模块行数围栏只用于阻止明显膨胀：`page.py` 与标注画布模块硬上限 350 行，共享 worker 硬上限 300 行，服务实现硬上限 400 行，服务包 `__init__.py` 硬上限 80 行。原 250/220/300 行阈值仅作为职责审查建议线，超过建议线时先判断内聚性和可读性；`src/ui/features/` 中超过 600 行的模块必须登记职责审查，超过已登记的 900 行安全线才导致结构测试失败。顶层 UI 类较多的模块必须显式拆分或登记，禁止通过兼容壳重新堆叠职责；禁止为通过测试压缩排版、删除合理空白或进行没有职责收益的拆文件。
...
## 1. 子 Agent 派发与并行策略 (Delegation & Subagents)

- **只读分析与探索（鼓励并行）**：
  在需要进行大型 PR 审查、复杂模块排查、安全扫描或全盘架构分析时，主 Agent **应该** 主动派发并行子 Agent（Subagent）分别处理不同模块，最后由主 Agent 汇总结论。
- **代码修改与写入（严格串行）**：
  **严禁** 开启多个子 Agent 同时修改源码文件，以防产生 Git 代码冲突和逻辑不一致。所有的代码编写与重构，必须在主线程中单线程或串行完成。

## 2. 协作规范与边界

- **只读权限限制**：负责探索和审查的子 Agent 默认仅拥有“只读”权限，不得擅自修改配置文件或源码。
- **信息汇总要求**：子 Agent 完成任务后，只需向主线程提交关键结论与 Diff 摘要，保持上下文简洁。
