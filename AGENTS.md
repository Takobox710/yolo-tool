...
当前主 GUI 为 `scr/yolo_workbench_qt/app.py` 的 PySide6 / Qt 实现，`scr/yolo_workbench/main.py` 只作为标准启动入口转发到 Qt。不要重新引入 CustomTkinter 主界面，也不要恢复 `app-tk` 任务。

Qt 迁移后的界面原则：

- 页面切换必须使用 Qt 页面栈或等效缓存机制，避免重新出现控件逐个加载的观感。
- 迁移不是重新设计信息架构；主页和模型训练页必须沿用旧版整体设计的信息结构、模块位置和交互约定。
- 可以使用 Qt 原生控件重建视觉细节，但不能把原型里的关键模块简化成大文本框或临时占位区。
...
- 主页采用 CSS Grid 两行两列布局，列宽比例 1:2，行高比例 58:42，四张卡片自动拉伸填充网格。
- 顶部保留欢迎标题、副标题，以及简短环境标签（左侧显示图标）。
- 上半区左侧为"项目概览"（标题与"设置项目目录"按钮同行），包含项目文件夹、图片路径、标注路径、结果路径、图片数量、标签数量。
- 上半区右侧为"各类别图片分布"，展示训练/验证/测试三大类照片的柱状图，显示类别、照片总数、训练/验证/测试数量。
- 下半区左侧为"训练曲线"，从 `results.csv` 读取数据并以不同颜色线条显示 Box Loss、Val Box Loss、mAP50、mAP50-95 等指标，右上角显示图例；暂无训练记录时显示空状态。
...
- 模型扫描与检测结果归一化。
- Qt 应用入口和核心功能迁移验证。
- 图标资源、网格布局、相对路径、训练曲线、悬停高亮、防重复启动、自定义命令框、系统信息样式等功能验证。
...
- 训练曲线已从 `results.csv` 读取数据绘制，后续可增加更多指标。
