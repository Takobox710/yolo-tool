...
- 顶部保留欢迎标题、副标题，以及简短环境标签。
- 上半区左侧为“项目概览”，包含项目文件夹、图片路径、标注路径、结果路径、图片数量、标签数量。
- 上半区右侧为“各类别图片分布”，使用 Qt 绘制或表格化展示类别/图片数量概览。
- 下半区左侧为“训练曲线”，暂无训练记录时显示空状态。
- 下半区右侧为“训练历史”或训练结果模型表格，自动扫描 `result/**/weights/*.pt`。
...
- 底部为训练日志面板，标题为“训练日志”，标题栏右侧显示短进度条和百分比。
- 日志区必须横向占满页面主内容宽度，不能被按钮区挤窄。
