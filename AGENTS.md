...
- 普通 Git 提交的 body 必须逐字等于归并后 CHANGELOG 提交条目下的正文，包含相同的项目符号、顺序和具体内容；每条优先精简表述，保留最终行为、关键对象和结果；项目符号数量按实际独立改动决定，不为减少条目强行合并或拆分。不得另写更短的总结或补充 CHANGELOG 没有的改动。提交前后必须核对两者一致；不一致时必须停止提交并修正。
