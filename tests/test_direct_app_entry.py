...
def test_project_path_helpers_display_relative_and_resolve_user_text(tmp_path):
    ...
    assert _display_project_path(str(inside), tmp_path) == str(
        Path("data") / "data.yaml"
    )
    ...
    assert (
        Path(_resolve_project_path(str(inside).replace(os.sep, "/"), tmp_path))
        == inside.resolve()
    )
...
def test_qt_app_uses_project_local_icon_assets():
    src = _read_app()
    assert (
        'icon_path = Path(__file__).resolve().parent / "assets" / "app_icon.png"' in src
    )
...
def test_training_path_fields_and_augment_order_are_locked():
    ...
    assert (
        'self.stacked_path_field("数据集YAML", training.get("data", ""), self.choose_file, "选择 data.yaml")'
        in src
    )
    assert (
        'self.stacked_path_field("模型YAML", "", self.choose_file, "可选，留空使用基础模型")'
        in src
    )
    assert (
        'self.stacked_path_field("项目输出", training.get("project", ""), self.choose_dir, "选择训练结果输出目录")'
        in src
    )
    assert (
        '[("mosaic", "马赛克"), ("scale", "缩放"), ("translate", "平移"), ("hsv_h", "HSV"), ("fliplr", "左右翻转"), ("flipud", "上下翻转"), ("degrees", "旋转"), ("mixup", "MixUp")]'
        in src
    )
...
def test_validation_detection_toolbar_modes_and_table_columns():
    ...
    assert (
        '[("上一张", self.prev_result), ("下一张", self.next_result), ("第一张", self.first_result), ("最后一张", self.last_result), ("列表", self.show_result_list), ("打开保存文件夹", self.open_detection_save_dir)]'
        in src
    )
...
def test_preview_tab_uses_validation_style_list_dialog():
    src = _read_app()
    assert (
        'for text, slot in [("扫描", self.load_preview_items), ("上一张", self.prev_image), ("下一张", self.next_image), ("列表", self.show_preview_list)]'
        in src
    )
    assert "def show_preview_index(self, index: int):" in src
    assert "def show_preview_list(self):" in src
    assert 'dialog.setWindowTitle("图片列表")' in src
    assert "dialog.resize(320, 520)" in src
    assert "dialog.setMinimumSize(200, 200)" in src
    assert 'search.setPlaceholderText("搜索文件名")' in src
    assert (
        "visible_paths = [path for path in self.preview_items if not needle or needle in path.name.lower()]"
        in src
    )


def test_rename_padding_parser_treats_blank_as_no_zero_fill():
    from scr.yolo_workbench_qt.app import _parse_padding_text

    assert _parse_padding_text("") == 0
    assert _parse_padding_text("   ") == 0
    assert _parse_padding_text("3") == 3
    assert _parse_padding_text("1") == 1


def test_camera_mode_uses_live_detection_helpers():
    from scr.yolo_workbench_qt.app import (
        _build_detection_log_message,
        _is_live_source_mode,
        _should_store_detection_history,
    )

    assert _is_live_source_mode("摄像头") is True
    assert _is_live_source_mode("图片/视频") is False
    assert _should_store_detection_history("摄像头") is False
    assert _should_store_detection_history("图片/视频文件夹") is True
    message = _build_detection_log_message(
        {
            "status": "摄像头",
            "elapsed": 0.05,
            "items": [object(), object()],
    ...
    assert "实时帧率" in message
    assert "FPS: 20.0" in message
    assert "结果: 2 个" in message
...
def test_detection_batch_navigation_keeps_user_selected_result_visible():
    ...
    assert (
        "self.is_batch_detection = not _is_live_source_mode(self.mode_combo.currentText())"
        in src
    )
...
def test_rename_and_camera_regressions_are_locked():
    src = _read_app()
    rename_section = src.split("class RenameTab(BasePage):", 1)[1].split(
        "class ResizeTab(BasePage):", 1
    )[0]
    assert "self.labelme_box, self.labelme_edit = self.path_field(" in rename_section
    assert '"Labelme 标注文件夹"' in rename_section
    assert "self.yolo_box, self.yolo_edit = self.path_field(" in rename_section
    assert '"YOLO 标注文件夹"' in rename_section
    assert "self.padding_box, self.padding_combo = self.combo_field(" in rename_section
    assert '"编号位数", "1", ["1", "2", "3", "4"]' in rename_section
    assert "grid.addWidget(widget, index // 3, index % 3)" in rename_section
    assert 'self.include_labelme = QCheckBox("Labelme 标注文件一并更改")' in rename_section
    assert 'self.include_yolo = QCheckBox("YOLO 标注文件一并更改")' in rename_section
    assert "self.include_labelme.setChecked(False)" in rename_section
    assert "self.include_yolo.setChecked(False)" in rename_section
    assert "self.table = QTableWidget(0, 3)" in rename_section
    assert "self.table.setHorizontalHeaderLabels(" in rename_section
    assert '"图片文件状态", "Labelme 标注状态", "YOLO 标注状态"' in rename_section
    assert "def image_status(self, item) -> str:" in rename_section
    assert 'return f"{item.old_name} -> {item.new_name}"' in rename_section
    assert "table_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)" in rename_section
    assert "preview_button = QPushButton(" not in rename_section
    assert 'run_button = QPushButton("执行重命名")' in rename_section
    assert "actions.addWidget(run_button)" in rename_section
    assert "actions.addWidget(preview_button)" not in rename_section
    assert 'QTimer.singleShot(100, self.preview)' in rename_section
    assert "key=natural_sort_key" in src
    assert "if _is_live_source_mode(self.mode_combo.currentText()):" in src
    assert 'self.counter.setText("实时预览")' in src
    assert "self.set_result_navigation_enabled(not camera)" in src
...
def test_qt_metric_cards_keep_border_style():
    ...
    assert (
        "QTableWidget::item { padding: 6px; border-bottom: 1px solid #E8EDF2; }" in src
    )
...
def test_conversion_tab_explains_labelme_multi_class_and_split_controls():
    src = _read_app()
    for expected in [
        '"数据集与转换配置"',
        '"转换参数"',
        'self.labelme_check = QCheckBox("Labelme 转 YOLO (?)")',
        "self.yolo_labels_box, self.yolo_labels_edit",
        "hint_label(",
        "setToolTip(",
        ...
        assert expected in src
    assert "card.layout.addLayout(content_layout)" in src
    assert "card.layout().addLayout(content_layout)" not in src
    assert 'left_card = Card("数据集与转换配置")' in src
    assert 'right_card = Card("转换参数")' in src
    assert "top_row.addWidget(left_card, 3)" in src
    assert "top_row.addWidget(right_card, 2)" in src
    assert "param_grid.addWidget(self.task_box, 0, 0)" in src
    assert "param_grid.addWidget(self.train_ratio_box, 0, 1)" in src
    assert "param_grid.addWidget(self.val_ratio_box, 1, 0)" in src
    assert "param_grid.addWidget(self.test_ratio_box, 1, 1)" in src
    assert "param_grid.addWidget(self.seed_box, 2, 0)" in src
    assert "param_grid.addWidget(line_box, 2, 1)" in src
    assert "class_names=[]" in src
    assert "self.classes_edit" not in src
    assert 'Card("类别名称")' not in src
    assert "hint_path_field(" not in src
    for removed_text in [
        "开启时读取同名 .json 并转换",
        "自动识别类别，结果输出会列出类别名称与统计",
        "合计:",
        "OBB + Labelme 直线标注时按半宽扩展为旋转矩形。",
    ]:
        assert removed_text not in src
