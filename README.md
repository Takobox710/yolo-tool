...
- 每次启动后首次进入系统设置页会在后台检查 GitHub 最新稳定 Release；后续切换页面不重复检查，发现高于当前 `1.3.3` 的版本时程序版本卡片后显示升级图标，并将该 Release 的更新内容追加到下方程序日志，网络不可用不会阻塞使用。点击程序版本号或升级图标可打开更新窗口，窗口支持分别勾选程序安装包、基础环境包和附加环境包；环境包更新按 Release 文件名中的 `vX` 版本与本机安装清单或包信息版本比较，源码版回退读取 `installer/*-version.txt`，Release 带有同版本环境包时不误报环境更新，基础包缺失按环境缺失处理，附加包缺失只显示可选下载安装提示，只有已安装附加包版本较旧时才触发环境更新；检测到环境包更新时会在进度模块上方显示环境更新信息模块，勾选资源会一并下载到用户 `Downloads` 文件夹；程序与更高版本基础环境同时需要更新时两者自动勾选，仅程序需要更新时只自动勾选程序包，程序-only 且未勾选环境包时同版本提示使用普通文字，手动勾选同版本基础包时显示红色重装提醒，基础包单独留下时在进度条下方显示红色不可安装提醒；附加环境包可单独下载，也可与程序安装包同时勾选，并根据是否已有附加包显示自动安装或替换提示，三项全选时合并显示基础包重装和附加包替换状态，已有附加包时下载前确认替换；仅更新程序时会根据环境包状态提示风险或重装选项，勾选程序安装包时下载完成后自动运行安装器，下载按钮右侧可暂停/继续下载或安装，下载期间窗口不可关闭。
