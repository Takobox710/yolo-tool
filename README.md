...
- 绘制预览和所有标注模式中的已完成标注都显示圆点；绘制模式只展示角点、不允许交互。编辑模式悬浮到标注框、内部或具体角点时显示与选中态深度一致的背景，且仅悬浮角点显示方块；移开后未选中标注恢复无背景，选中标注继续显示同等深度的半透明背景。
...
- 标注控制点固定为直径 `9 px`，不随图片分辨率变化；鼠标命中范围略大于可视控制点。
