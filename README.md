...
| 数据处理 | Labelme 转 YOLO、YOLO 原生数据集划分、标注预览、批量重命名；图片压缩支持画布补边或中心裁剪、比例和输出分辨率。 |
...
3. **整理数据集**：在数据处理中选择 Labelme 转换模式或 YOLO 原生划分模式。默认比例为 `train=0.8`、`val=0.2`、`test=0.0`，空 split 不预建目录；预览和执行在后台运行并显示进度。
...
5. **验证结果**：模型验证默认显示训练产物中的 `best.pt`；开启“模型验证显示 last”后才显示 `last.pt`。验证不提供图片尺寸设置，始终使用模型原生推理尺寸；普通图片检测默认输出到 `result/gui_val`，标签位于输出目录的 `labels/` 子目录；视频输出为 MP4。
