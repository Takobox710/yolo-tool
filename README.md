...
- `标签文件夹` 当前用于 Labelme `.json` 标注目录；YOLO 标注目录在“更多设置”中配置。
...
- 点击“画标注框”后，会先弹出标注类型选择窗口；窗口最上方单独提供 `编辑` 选项，并与下方绘制类型之间使用分隔线隔开。
- 右键菜单当前顺序为：`编辑`、分隔线、`矩形框`、`有向矩形`、`镜像有向矩形`、`多边形`、`圆形`、`直线拓展`、分隔线、`删除选中框`、`取消当前绘制`。
- 选中标注后支持直接拖动、拖动控制点修改形状，并支持 `Delete` 删除。
- 除编辑模式外，选择任意绘制模式后画布显示系统十字光标辅助定位；矩形框模式额外显示贯穿画布的纯黑色实线水平/垂直辅助线，并在短光标周围留出原始背景采样空隙；多边形悬停到封闭顶点时仍显示小手。
- 标注控制点固定为直径 `10 px`，不随图片分辨率变化；鼠标命中范围略大于可视圆点。
