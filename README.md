...
- 内置本地手工数据标注页，默认读写 Labelme `.json`，支持右键切换绘制/选择模式、类别管理、AI 预标注与项目级标注设置。
...
- 点击“画标注框”后，会先弹出标注类型选择窗口，再进入对应绘制模式。
- 右键菜单当前顺序为：`选择`、分隔线、`矩形框`、`有向矩形`、`镜像有向矩形`、`多边形`、`圆形`、`直线拓展`、分隔线、`删除选中框`、`取消当前绘制`。
- 选中标注后支持直接拖动、拖动控制点修改形状，并支持 `Delete` 删除。
- 圆形标注采用“两次点击确认”的方式，不依赖按住鼠标再松开。
- `Esc` 支持两级取消：先取消当前正在绘制的标注；若当前未在绘制，则退回到“选择”模式。
- “更多设置”当前包含：
  - 是否自动保存 Labelme `.json`
  - 是否自动转换并保存 YOLO `.txt`
  - YOLO 标注文件夹
