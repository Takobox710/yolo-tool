...
- 每次启动后首次进入系统设置页会在后台检查 GitHub 最新稳定 Release；后续切换页面不重复检查，发现高于当前 `1.3.1` 的版本时程序版本卡片后显示升级图标，并将该 Release 的更新内容追加到下方程序日志，网络不可用不会阻塞使用。点击程序版本号或升级图标可打开更新窗口，窗口支持分别勾选程序安装包、基础环境包和附加环境包；检测到环境包更新时会在进度模块上方显示环境更新信息模块，勾选资源会一并下载到用户 `Downloads` 文件夹；程序与基础环境同时需要更新时两者自动勾选，仅程序需要更新时只自动勾选程序包，基础包单独留下时在进度条下方显示红色不可安装提醒；附加环境包可单独下载并在程序内热安装或替换，单独选择附加包时按钮显示“下载并安装所选资源”；仅更新程序时会根据环境包状态提示风险或重装选项，勾选程序安装包时下载完成后自动运行安装器，下载按钮右侧可暂停/继续下载或安装，下载期间窗口不可关闭。
