...
- `预览划分` 和 `执行划分` 均通过后台任务执行，运行期间两个按钮禁用并显示当前阶段百分比，避免扫描、解析或复制大目录时阻塞界面。

图片压缩支持：

- 图片压缩的源图片目录使用独立的 `image_resize.source_dir`，修改后不改变上述共享图片目录。
- 可选是否备份原始图片，默认不备份。
- 支持“画布压缩”和“裁剪”两种处理方式，默认“画布压缩”。
- 两种方式都可选择 `1:1`、`4:3`、`16:9`、`3:4`、`9:16` 图片比例，并填写与比例一致的“宽×高”尺寸，例如 `640×480`。
- “画布压缩”按原图比例缩放，在目标比例的画布中居中补白或补黑；该尺寸字段显示为“画布尺寸”，背景颜色可选白色或黑色。
- “裁剪”从图片中心按所选比例裁取区域，再缩放至目标尺寸；该尺寸字段显示为“输出分辨率”，不使用背景颜色。
- 预览结果显示每张图片的原始尺寸、输出尺寸、缩放倍率，以及裁剪模式的原图裁剪区域。
