...
- 左侧卡片标题为"数据集划分配置"，上方 2x2 放置图片目录、Labelme 标注目录、YOLO 标注目录、输出目录；下方放置"Labelme 转 YOLO"、"备份标注文件"和"自定义类别名称"。
...
- 数据集划分页仅以下项目显示 `ⓘ`：`Labelme 转 YOLO`、`备份标注文件`、任务类型、训练、验证、测试、随机种子、直线拓展宽度。
...
- ONNX、TorchScript、OpenVINO 和 NCNN 使用基础安装环境。TensorRT 在发布版中需要另行安装模型转换环境包；开发态 Pixi 环境直接具备全部能力。
- 模型转换附加包发布名为 `YOLOTool_ExtraEnv_<版本>.7z`，同时兼容同结构 `.zip`，不执行第三方安装器；已安装旧版本时必须先确认替换。
- 附加包安装显示阶段进度，拒绝路径穿越、绝对路径、符号链接、错误平台、协议不匹配和 SHA-256 不匹配；安装失败继续使用旧版本，成功后只保留当前版本和一个上一版本。
