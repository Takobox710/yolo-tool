...
- 训练任务通过 `WorkbenchContext.tasks` 获取 `train` 租约；已有训练任务时拒绝重复启动。停止、完成和异常退出都释放租约，旧页面或旧项目 generation 的回调不得恢复新任务控件。
...
- 训练日志框为只读展示控件，但必须允许鼠标选中文本并使用 `Ctrl+C` 复制。
- 开始训练前应检查并修复 `data.yaml` 中验证集 `val:` 的残留异常路径，例如把反斜杠路径恢复为与 `train:` 对应的验证路径，再生成最终训练命令。
