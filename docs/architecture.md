...
- 安装器和程序内安装服务均支持基础包和模型转换附加包的单卷 `.7z` 与完整两卷 `.7z.001/.002`；选择或拖入 `.002` 时自动规范化为同目录 `.001` 首卷，分卷依赖原生 7-Zip，自动发现时单卷优先，缺少 `.002` 或存在 `.003` 的分卷拒绝使用。`package_windows.ps1` 生成 Program 安装器时同样优先登记单卷，单卷缺失才登记分卷首卷。
