...
- `(root)`: 9 个文件，1979 行文本；仓库根目录入口与说明文件。
- `scr/services`: 15 个文件，3043 行文本；服务层，可测试业务逻辑。
- `scr/ui/views`: 29 个文件，6477 行文本；各功能页面与训练/验证页拆分模块。
- `scr/ui/widgets`: 3 个文件，421 行文本；共享控件。
- `scr/ui`: 9 个文件，1264 行文本；主窗口、页面基类、表单混入、工作线程与 UI 辅助。
- `scr/tests`: 21 个文件，3958 行文本；pytest 测试。
- `docs/spec`: 6 个文件，273 行文本；功能规格文档。
- `docs`: 3 个文件，375 行文本；架构、清单与打包文档。
...
| `docs/architecture.md` | 205 | 项目架构、分层边界、维护阈值与拆分策略说明。 |
| `docs/code-inventory.md` | 138 | 仓库代码与结构清单，统计主要文件行数与职责。 |
| `docs/packaging-windows.md` | 32 | Windows 打包命令与推荐模式说明。 |
| `docs/spec/annotation.md` | 48 | 数据标注页交互与布局规格。 |
| `docs/spec/data-processing.md` | 47 | 数据处理子模块规格。 |
| `docs/spec/home.md` | 35 | 主页信息布局与展示规格。 |
| `docs/spec/settings.md` | 20 | 系统设置页规格与系统信息要求。 |
| `docs/spec/training.md` | 67 | 训练页字段、模型选择与运行交互规格。 |
| `docs/spec/validation.md` | 56 | 验证页布局、模式与结果展示规格。 |
...
| `scr/main.py` | 57 | 统一入口，负责 GUI/隐藏 CLI 启动分发。 |
...
| `scr/services/annotation_ai_service.py` | 273 | AI 预标注服务，负责模型解析、图片筛选与批量标注写回。 |
...
| `scr/services/detection_service.py` | 449 | 验证/检测模型扫描、输入源收集、结果提取、日志文案与结果保存服务。 |
| `scr/services/editable_annotation_service.py` | 287 | 可编辑标注与 Labelme/YOLO 双向读写服务。 |
| `scr/services/environment_service.py` | 131 | Pixi、Torch/CUDA 与系统环境探测服务。 |
...
| `scr/services/runtime_service.py` | 162 | 后台进程启动、日志清洗与停止控制服务。 |
...
| `scr/tests/test_services_environment.py` | 132 | 环境探测服务测试。 |
...
| `scr/tests/test_ui_training.py` | 334 | 训练页模型选择、持久化与停止流程测试。 |
| `scr/tests/test_ui_validation.py` | 493 | 验证页模型列表、模式切换与检测流程测试。 |
| `scr/theme.py` | 59 | 全局 Qt 主题样式表。 |
| `scr/train_cli.py` | 368 | 打包后训练、导出、验证命令行入口。 |
...
| `scr/ui/views/annotation_ai_dialog.py` | 613 | AI 预标注主对话框，配置模型、范围、映射与执行。 |
...
| `scr/ui/views/settings.py` | 269 | 系统设置页面。 |
| `scr/ui/views/training.py` | 88 | 训练页面入口，负责页面装配与方法兼容导出。 |
| `scr/ui/views/training_form.py` | 218 | 训练页表单、状态卡和日志面板布局构建。 |
| `scr/ui/views/training_runtime.py` | 115 | 训练启动、停止、日志轮询与结果目录打开逻辑。 |
| `scr/ui/views/training_state.py` | 278 | 训练配置收集、设置持久化和命令预览逻辑。 |
...
| `scr/ui/views/validation_results.py` | 105 | 检测结果渲染与缓存结果回显逻辑。 |
| `scr/ui/views/validation_runtime.py` | 195 | 验证页检测启动、停止、worker 管理与数据集验证桥接逻辑。 |
...
| `scr/ui/window.py` | 207 | 主工作台窗口、导航与页面装配。 |
| `scr/ui/workers.py` | 236 | 线程工作器，封装检测与 AI 预标注后台任务。 |
