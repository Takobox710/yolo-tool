...
- `src`: 248 个文件，27798 行文本；主源码目录，包含入口、共享层、服务层、UI 与测试。
- `docs`: 8 个文件，795 行文本；架构、规格、打包与代码清单文档。
- `installer`: 12 个文件，2046 行文本；Windows 打包脚本、PyInstaller 与 Inno Setup 配置。
...
| `README.md` | 412 | 项目概览、命令入口与使用说明。 |
| `pixi.toml` | 61 | Pixi 环境、依赖与任务命令定义。 |
...
| `src/bootstrap/cli_dispatch.py` | 104 | 启动装配、GUI/CLI 分发与应用上下文入口。 |
| `src/bootstrap/handlers.py` | 43 | 启动装配、GUI/CLI 分发与应用上下文入口。 |
| `src/data/runtime/app_state.json` | 3 | 仓库文件。 |
| `src/data/runtime/settings.json` | 119 | 仓库文件。 |
| `src/devtools/__init__.py` | 1 | 仓库文件。 |
| `src/devtools/base_runtime_package.py` | 43 | 仓库文件。 |
| `src/devtools/companion_catalog.py` | 94 | 仓库文件。 |
| `src/devtools/generate_code_inventory.py` | 151 | 仓库文件。 |
| `src/devtools/model_export_package.py` | 186 | 仓库文件。 |
| `src/devtools/package_cache.py` | 57 | 仓库文件。 |
| `src/devtools/release_package.py` | 356 | 仓库文件。 |
| `src/main.py` | 53 | 仓库文件。 |
...
| `src/services/annotation/editable_document.py` | 300 | 服务层与可测试业务逻辑实现。 |
...
| `src/services/settings/model.py` | 315 | 服务层与可测试业务逻辑实现。 |
...
| `src/tests/integration/test_installer_contract.py` | 153 | pytest 测试、结构约束与回归用例。 |
| `src/tests/integration/test_model_export_packaging.py` | 46 | pytest 测试、结构约束与回归用例。 |
...
| `src/tests/services/model_export/test_package_collector.py` | 89 | pytest 测试、结构约束与回归用例。 |
...
| `src/tests/services/runtime/test_release_manifest.py` | 204 | pytest 测试、结构约束与回归用例。 |
...
| `src/ui/features/annotation/ai/dialog.py` | 621 | 按功能分包的页面真实实现。 |
...
| `src/ui/features/annotation/ai/preferences.py` | 79 | 按功能分包的页面真实实现。 |
...
| `src/ui/features/annotation/canvas/render.py` | 211 | 按功能分包的页面真实实现。 |
| `src/ui/features/annotation/canvas/state.py` | 25 | 按功能分包的页面真实实现。 |
| `src/ui/features/annotation/canvas/status.py` | 21 | 按功能分包的页面真实实现。 |
| `src/ui/features/annotation/canvas/widget.py` | 238 | 按功能分包的页面真实实现。 |
| `src/ui/features/annotation/class_panel.py` | 82 | 按功能分包的页面真实实现。 |
| `src/ui/features/annotation/dialogs.py` | 543 | 按功能分包的页面真实实现。 |
...
| `src/ui/features/annotation/settings_actions.py` | 119 | 按功能分包的页面真实实现。 |
...
| `src/ui/features/settings/page.py` | 96 | 按功能分包的页面真实实现。 |
| `src/ui/features/settings/state.py` | 182 | 按功能分包的页面真实实现。 |
...
| `src/ui/shared/context.py` | 130 | 跨页面复用的表单、对话框与页面基类。 |
...
| `src/ui/shell/close_guard.py` | 35 | 主窗口壳层、样式与页面协调。 |
...
| `docs/architecture.md` | 236 | 项目架构、打包与维护文档。 |
| `docs/packaging-windows.md` | 116 | 项目架构、打包与维护文档。 |
| `docs/spec/annotation.md` | 133 | 页面与功能规格说明。 |
| `docs/spec/data-processing.md` | 84 | 页面与功能规格说明。 |
| `docs/spec/home.md` | 43 | 页面与功能规格说明。 |
| `docs/spec/settings.md` | 40 | 页面与功能规格说明。 |
...
| `installer/build_base_runtime_models.ps1` | 45 | Windows 打包脚本与安装配置。 |
| `installer/build_model_export_runtime.ps1` | 39 | Windows 打包脚本与安装配置。 |
...
| `installer/package_windows.ps1` | 194 | Windows 打包脚本与安装配置。 |
| `installer/runtime-version.txt` | 1 | Windows 打包脚本与安装配置。 |
| `installer/yolo_tool.iss` | 1346 | Windows 打包脚本与安装配置。 |
| `installer/YOLOTool.spec` | 214 | Windows 打包脚本与安装配置。 |
