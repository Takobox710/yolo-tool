...
from src.bootstrap.cli_common import _emit_structured as _default_emit, _load_json_payload
from src.services.training import infer_task_mode_from_model

def _run_predict_cli_impl(argv: list[str], emit: Callable[..., None] | None = None) -> int:
    ...
    if mode in {"图片检测", "视频检测", "图片文件夹", "视频文件夹", "图片/视频文件夹", "图片/视频"}:
        ...
        emit("progress", message=f"已找到 {len(paths)} 个待检测文件。")
    else:
        paths = []
        emit("progress", message=f"正在打开摄像头 {config.get('camera_index', 0)}。")

    emit("progress", message=f"正在加载模型：{Path(model_path).name}")
    ...
    emit("progress", message=f"检测结果将保存到：{save_dir}")
    ...
    def predict_image(image_path: Path, index: int, total: int) -> None:
        emit("progress", message=f"正在检测 {index}/{total}：{image_path.name}")
        import time
