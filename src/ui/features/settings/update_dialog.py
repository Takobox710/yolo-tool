...
class ReleaseUpdateDialog(QDialog):
    ...
    def _build_layout(self) -> None:
        ...
        if self.result.environment_asset_names:
    ...
    def _confirm_download_selection(self) -> bool:
        ...
        if self.base_environment_checkbox.isChecked() or self.extra_environment_checkbox.isChecked():
            return True
        if _has_any_environment_asset(self.result):
        ...
        answer = QMessageBox.question(
            self,
            "环境包无需更新",
            "当前仅检测到程序更新，环境包无需更新。是否重装环境包？\n"
            "当前 Release 未提供新的环境包，确认后将继续使用现有环境。",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def _sync_pre_download_message(self) -> None:
        ...
        if self.base_environment_checkbox.isChecked() and not self.program_checkbox.isChecked():
            ...
            if self._installed_extra_environment() is not None:
                ...
                warning = False
        elif self.program_checkbox.isChecked() and _has_any_environment_asset(self.result):
            message = "当前 Release 同时提供环境包，仅更新程序可能导致部分功能无法使用，点击下载时会确认。"
            warning = True
        elif self.program_checkbox.isChecked():
            message = "当前 Release 未提供新的环境包，环境包无需更新，点击下载时会确认是否继续。"
            warning = True
        else:
            message = "安装包尚未下载。"
            warning = False
...
def _has_any_environment_asset(result: ReleaseCheckResult) -> bool:
    return _has_environment_asset(result, "baseenv") or _has_environment_asset(
        result, "extraenv"
    )
