...
def build_convert_layout(page) -> None:
    ...
    actions = QHBoxLayout()
    page.preview_button = QPushButton("预览划分")
    page.preview_button.clicked.connect(page.preview)
    page.run_button = QPushButton("执行划分")
    page.run_button.clicked.connect(page.run)
    actions.addWidget(page.preview_button)
    actions.addWidget(page.run_button)
