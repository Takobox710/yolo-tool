...
- `标注转换`、`标注预览`、`批量重命名` 和 `数据标注` 的图片目录/图片文件夹共用 `paths.images_dir`；`数据标注`、`标注转换` 和 `批量重命名` 的 Labelme 标注目录共用 `paths.annotations_dir`；`标注预览` 与 `标注转换` 的 YOLO 标注目录共用 `paths.labels_dir`。任一共享字段修改后，其他页面必须立即同步，不需要重启程序。
